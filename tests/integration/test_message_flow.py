...
async def test_changes_collection_hash_chain(db, org_id, actor):
    ...
    # Verify chain
    records = await ChangeRecord.find({"org_id": org_id}).sort("timestamp").to_list()
...
async def test_message_claim_and_complete(db, org_id, actor):
    ...
    # Log should have it
    log_entries = await MessageLog.find({"correlation_id": "test-corr-1"}).to_list()
...
async def test_message_fail_and_dead_letter(db, org_id, actor):
    ...
    await bus.claim_by_id(msg.id, actor.id)
