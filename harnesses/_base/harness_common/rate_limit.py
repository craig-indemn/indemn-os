"""In-process sliding-window rate limiter shared by voice + chat frontdoors
(AI-408 Phase 3 extraction of voice-frontdoor's AI-407 Task 2.36 impl per
§10.7 row "Replay of session creation").

**Ordering invariant:** rate-limit MUST fire BEFORE Interaction creation
+ LiveKit/checkpointer resource allocation. Otherwise an attacker
exhausts resource slots + writes audit-trail records before being
throttled — defeating the mitigation.

v1 — single-container in-memory windows per-IP / per-actor / per-Deployment.
When either frontdoor scales beyond one Railway instance, replace with a
...
Per-surface wrappers (`harnesses/voice-frontdoor/harness/rate_limit.py`
re-exports for back-compat; `harnesses/chat-deepagents/main.py` imports
directly) instantiate their own module-level singleton so the sliding
state is per-process per-surface (not shared between voice + chat —
each surface has its own throttle).
"""
...
class RateLimiter:
    ...
    def check_with_retry(
        ...
        429/1013 means slow this user; a per-IP 429/1013 means a whole
        office's NAT'd traffic is hot).
