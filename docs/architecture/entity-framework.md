...
| `bulk_update_tracked(entities, sets, ...)` | Batch update via `BulkExecuteWorkflow` UPDATE branch | Per-entity audit with in-memory hash chain; single insert_many for audit + single update_many for entities. **Watch events stay silent** for bulk updates (locked Session-36 Dev#3 as audit-only — bulk-update is administrative / migration so the no-event semantic matches operator expectation; symmetric with `bulk_delete_tracked`; asymmetric with `bulk_save_tracked` ingestion which IS noisy because downstream cascade SHOULD fire). |
